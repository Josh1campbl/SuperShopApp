// Dafny program Q3temp.dfy compiled into C#
...
// Dafny 3.2.0.30713
// Command Line Options: c:\Users\Josh\COMPSCI\SWEN324\Assignment2\A2Student\A2Student\Q3temp.dfy /verifyAllModules /compile:3 /spillTargetCode:1 /out:bin\Q3temp
// Q3temp.dfy
...
function method helper<T>(x: List<T>): Option<T>
  decreases x
{
  match x
  case Nil() =>
  ...
  decreases l
{
  ...
  case Nil() =>
    true
  case Cons(car, cdr) =>
    if (cdr.Cons? && car <= cdr.car) || cdr.Nil? then
      isPriorityQ(cdr)
    else
      false
}
...
method Main()
{
  ...
  assert second(l) == 2;
  assert forall x: List<nat> :: x.Cons? && x.cdr.Cons? ==> second(x) == secondOpt(x).value;
  assert forall x: List<nat> :: x.Nil? || x.cdr.Nil? ==> None == secondOpt(x);
...
namespace _module {
  ...
  public abstract class List<T> {
    ...
    public static List<T> create_Cons(T car, List<T> cdr) {
      return new List_Cons<T>(car, cdr);
    }
    ...
    public T dtor_car {
      get {
        var d = this;
        return ((List_Cons<T>)d).car; 
      }
    }
    public List<T> dtor_cdr {
      get {
        var d = this;
        return ((List_Cons<T>)d).cdr; 
      }
    }
  }
  ...
  public class List_Cons<T> : List<T> {
    ...
    public readonly List<T> cdr;
    public List_Cons(T car, List<T> cdr) {
      this.car = car;
      this.cdr = cdr;
    }
    public override bool Equals(object other) {
      var oth = other as List_Cons<T>;
      return oth != null && object.Equals(this.car, oth.car) && object.Equals(this.cdr, oth.cdr);
    }
    public override int GetHashCode() {
      ulong hash = 5381;
      hash = ((hash << 5) + hash) + 1;
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this.car));
      hash = ((hash << 5) + hash) + ((ulong)Dafny.Helpers.GetHashCode(this.cdr));
      return (int) hash;
    }
    public override string ToString() {
      string s = "List.Cons";
      s += "(";
      s += Dafny.Helpers.ToString(this.car);
      s += ", ";
      s += Dafny.Helpers.ToString(this.cdr);
      s += ")";
      return s;
    }
  }
  ...
  public abstract class Option<T> {
    ...
    public static Dafny.TypeDescriptor<Option<T>> _TypeDescriptor() {
      return new Dafny.TypeDescriptor<Option<T>>(Option<T>.Default());
    }
  ...
  public partial class __default {
    public static __T second<__T>(List<__T> l) {
      List<__T> _78_x = (l).dtor_cdr;
      List<__T> _source0 = _78_x;
      {
        __T _79___mcc_h0 = ((List_Cons<__T>)_source0).car;
        List<__T> _80___mcc_h1 = ((List_Cons<__T>)_source0).cdr;
        List<__T> _81_r = _80___mcc_h1;
        __T _82_l = _79___mcc_h0;
        return _82_l;
      }
    }
    public static Option<__T> secondOpt<__T>(List<__T> l) {
      List<__T> _source1 = l;
      if (_source1.is_Nil) {
        return @Option<__T>.create_None();
      } else {
        __T _83___mcc_h0 = ((List_Cons<__T>)_source1).car;
        List<__T> _84___mcc_h1 = ((List_Cons<__T>)_source1).cdr;
        List<__T> _85_r = _84___mcc_h1;
        __T _86_l = _83___mcc_h0;
        return __default.helper<__T>(_85_r);
      }
    }
    public static Option<__T> helper<__T>(List<__T> x) {
      List<__T> _source2 = x;
      if (_source2.is_Nil) {
        return @Option<__T>.create_None();
      } else {
        __T _87___mcc_h0 = ((List_Cons<__T>)_source2).car;
        List<__T> _88___mcc_h1 = ((List_Cons<__T>)_source2).cdr;
        List<__T> _89_r = _88___mcc_h1;
        __T _90_l = _87___mcc_h0;
        return @Option<__T>.create_Some(_90_l);
      }
    }
    public static bool isPriorityQ(List<BigInteger> l) {
    TAIL_CALL_START: ;
      List<BigInteger> _source3 = l;
      if (_source3.is_Nil) {
        return true;
      } else {
        BigInteger _91___mcc_h0 = ((List_Cons<BigInteger>)_source3).car;
        List<BigInteger> _92___mcc_h1 = ((List_Cons<BigInteger>)_source3).cdr;
        List<BigInteger> _93_cdr = _92___mcc_h1;
        BigInteger _94_car = _91___mcc_h0;
        if ((((_93_cdr).is_Cons) && ((_94_car) <= ((_93_cdr).dtor_car))) || ((_93_cdr).is_Nil)) {
          List<BigInteger> _in0 = _93_cdr;
          l = _in0;
    ...
    {
      List<BigInteger> _95_l;
      _95_l = @List<BigInteger>.create_Cons(BigInteger.One, @List<BigInteger>.create_Cons(new BigInteger(2), @List<BigInteger>.create_Cons(new BigInteger(3), @List<BigInteger>.create_Nil())));
      List<BigInteger> _96_np;
      _96_np = @List<BigInteger>.create_Cons(BigInteger.One, @List<BigInteger>.create_Cons(new BigInteger(3), @List<BigInteger>.create_Cons(BigInteger.One, @List<BigInteger>.create_Nil())));
    }
  }
